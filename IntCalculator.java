 ...
 * @author Shinjini Mukherjee sm5160
...
public class IntCalculator {
    ...
    public String infixToPostfix() throws StackException {
        ...
        for(int i = 0; i < len; i++) {
            ...
            //check multidigit and single digit
            if(isDigit(c)) {
                postfixExpression.append(c);

                    while (i + j < len && isDigit(infixExpression.charAt(i + j))) {
                        postfixExpression.append(infixExpression.charAt(i + j));
                        j++;
                }
    ...
    public int evaluatePostfix()
        ...
        if (len == 1 && isDigit(postfixExpression.charAt(0))){
        ...
        for(int i = 0; i < len; i++) {
            ...
            if (isDigit(c) && isDigit(postfixExpression.charAt(i + j))) {
                multi.append(ch);
                while (i + j < len && isDigit(postfixExpression.charAt(i + j))) {
                    multi.append(postfixExpression.charAt(i + j));
                    j++;
                }
                i = i + j - 1;
                String multiDig = multi.toString();
                int m = Integer.parseInt(multiDig);
                stack.push(m);
            }

            else if(isDigit(c) && !isDigit(postfixExpression.charAt(i + j))){
                stack.push(ch);
            }
                    ...
                    if (c == '^') {
                        ...
                        double one = (double)popOne;
                        double two = (double)popTwo;
                        int sol = (int)Math.pow(two, one);
                        stack.push(sol);
                    } else if (c == '/') {
                        if(popOne == 0) {
                            throw new IllegalArgumentException("Cannot evaluate expression, division by zero.");
                        }
                        else {stack.push(popTwo / popOne);}
                    } else if (c == '%') {
                        if(popOne == 0) {
                            throw new IllegalArgumentException("Cannot evaluate expression, division by zero.");
                        }
